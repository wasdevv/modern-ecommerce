...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b bg-white/90 backdrop-blur">
      <nav className="mx-auto flex max-w-6xl items-center gap-4 px-4 sm:gap-6 py-3">
        ...
        <Link href="/cart" className="ml-auto rounded-md border px-3 py-1.5 text-sm font-medium hover:bg-gray-50">
          Cart{hydrated && count > 0 && <span className="ml-1.5 rounded-full bg-gray-900 px-2 py-0.5 text-xs text-white">{count}</span>}
        </Link>
      </nav>
    </header>
  );
