  ...
  return (
    <>
      ...
      <div className="mt-4 grid gap-8 md:grid-cols-2">
        ...
        <div>
          <p className="text-sm uppercase tracking-wide text-gray-500">{product.category.replace('-', ' ')}</p>
          <h1 className="mt-1 text-3xl font-bold">{product.name}</h1>
          <p className="mt-2 text-sm text-gray-600">
            ★ {product.rating.toFixed(1)} · {product.reviewCount} reviews <span className="text-gray-500">(demo data)</span>
          </p>
          <div className="mt-6">
            <Price product={product} large />
          </div>
          <p className="mt-6 text-gray-700">{product.description}</p>
          {!product.inStock && <p className="mt-4 text-sm font-medium text-red-700">Enrollment is closed for this course.</p>}
