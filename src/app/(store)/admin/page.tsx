...
export default function AdminPage({ searchParams }: { searchParams: { error?: string } }) {
  ...
  return (
    <>
      <h1 className="text-[32px] md:text-[40px]">Painel</h1>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {kpis.map(([label, value]) => (
          <div key={label} className="border border-ink/10 bg-white p-5">
            ...
            <p className="mt-1 text-2xl font-normal text-ink">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        <section className="border border-ink/10 bg-white p-6">
          <h2 className="font-normal text-ink">Receita por mês</h2>
          <ul className="mt-4 space-y-3">
            {a.revenueByMonth.map((m) => (
              <li key={m.month} className="grid grid-cols-[4rem_1fr_7rem] items-center gap-3 text-sm">
                <span>{m.month}</span>
                <span className="h-3 bg-ink" style={{ width: `${(m.cents / maxMonth) * 100}%` }} />
        ...
        <section className="border border-ink/10 bg-white p-6">
          <h2 className="font-normal text-ink">Produtos com maior receita</h2>
          <ol className="mt-4 divide-y text-sm">
            {a.topProducts.map((p) => (
              <li key={p.productId} className="flex justify-between gap-4 py-2">
                <span>
                  {p.name} <span className="text-ink/75">· {p.units} vendidos</span>
                </span>
